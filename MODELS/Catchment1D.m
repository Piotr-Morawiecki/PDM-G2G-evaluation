  ...
  methods
    ...
    function [obj, par] = setParametersDimensional(obj, par)
      try
        ...
        par.rho = obj.par.rho;
        par.rho0 = obj.par.rho0;
        par.sigma = obj.par.sigma;
        par.mu = obj.par.mu;
        par.rk = obj.par.rk;
    ...
    %% findPeriodicSolution

    % Function findPeriodicSolution() finds a periodic solution
    ...
    function [h0, sol] = findPeriodicSolution(obj, h0, xmesh, tspan, ...
        max_periodic_error, max_periods)
      ...
      while periodic_error > max_periodic_error && iteration <= max_periods
        ...
        sol = obj.solve(h0, xmesh, tspan);
        
        h0 = sol(end, :);
        
        % Check max difference between the hydraulic head in the initial
        % and final time step
        periodic_error = max(abs(h_pervious - h0));
    ...
    function [sol, t_sat, h0] = solve(obj, h0, xmesh, tspan, stopWhenSat)
      ...
      % saturated case is used from the start.
        ...
        options = odeset('RelTol', 1e-8, 'AbsTol', 1e-8);
        sol = pdepe(0, ...
          @(x, t, h, dhdx) pde_model(x, t, h, dhdx, obj.par), ...
          @(x) ic(x, xmesh, h0), ...
          @(xl, hl, xr, hr, t) bc_sat(hl, obj.par), ...
          xmesh, tspan, options);
      end
    ...
    %
    % INPUT:
    %   xmesh - location of mesh elements (or single number representing
    %           number of elements if uniform mesh is used),
    %   h     - height h(x).
    %
    % OUTPUT:
    %   Q_dimless  - dimensionless river inflow (composed of the
    %                groundwater flow and overland flow)
    %   Qs_dimless - overland component of river inflow (dimensionless)
    %   Qg_dimless - groundwater component of river inflow (dimensionless)
    
    function [Q_dimless, Qs_dimless, Qg_dimless] = computeFlow(obj, xmesh, h)
      if length(xmesh) == 1
        xmesh = linspace(0, 1, xmesh);
      end
      ...
      Q_dimless = 1 + obj.par.mu * h .^ (5/3) .* (h>0) + ...
        obj.par.sigma * dhdx;
      Qs_dimless = obj.par.mu * h .^ (5/3) .* (h>0);
      Qg_dimless = 1 + obj.par.sigma * dhdx;
    end
...
function [c,f,s] = pde_model(x, t, h, dhdx, par)
  h_surf = h .* (h > 0);
  h_ground = 1 + h .* (h < 0);
  f = h_ground * (1 + par.sigma * dhdx) + par.mu * h_surf .^ (5/3);
  dh = 0;
  c = par.f(x) * (h <= dh) + (h > dh);
  if isnumeric(par.rho)
    s = par.rho;
  else
    s = par.rho(t);
  end
